...
class GA2010CorrelationModel(BaseCorrelationModel):
    ...
    def _get_correlation_model(self, distances, imt):
        ...
        if isinstance(imt, SA):
            if imt.period == 0.1:
                alpha = 0.062; beta = 0.276; gamma = 5.0
            elif imt.period == 0.2:
                alpha = 0.073; beta = 0.248; gamma = 5.0
            elif imt.period == 0.3:
                alpha = 0.086; beta = 0.219; gamma = 5.0
            elif imt.period == 0.5:
                alpha = 0.073; beta = 0.248; gamma = 5.0
            elif imt.period == 1.0:
                alpha = 0.051; beta = 0.329; gamma = 5.0
            elif imt.period == 2.0:
                alpha = 0.061; beta = 0.421; gamma = 3.035
            elif imt.period == 3.0:
                alpha = 0.092; beta = 0.671; gamma = 1.189
            elif imt.period == 5.0:        
                alpha = 0.071; beta = 0.741; gamma = 1.201
        else:
            assert isinstance(imt, PGA), imt
            alpha = 0.060; beta = 0.283; gamma = 5.0

        # formulae are from page 3063
        rho = numpy.empty_like(distances)
        [M,N] = numpy.shape(distances)
        
        for i in range(0,M):
            for j in range(0,N):
                rho[i,j] = max(gamma*numpy.exp(-alpha*numpy.power(distances[i,j],beta))-gamma + 1.0, 0.0)
